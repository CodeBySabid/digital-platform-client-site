    ...
    const handleGoogleRegistration = () => {
        signinGoogle()
            .then(result => {
                ...
                .then(res => {
                    console.log('after users data has been stor', res.data)
                    navigate(from, {replace: true});
                })
                .catch(error => {
                    console.log(error);
    ...
    return (
        <div>
            <div className="w-[320px] max-w-md p-8 rounded-2xl bg-white/10 backdrop-blur-md shadow-xl">
                ...
                {/* Register */}
                <p className="text-center mt-2 text-sm">Don't have any account?<Link state={{ from }} to={'/register'} className="font-semibold hover:text-[#3251ff] text-[#97b43e] cursor-pointer"> Register</Link></p>
